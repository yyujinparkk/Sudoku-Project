...
class Board:
    def __init__(self, width, height, screen, difficulty):
        ...
        self.cells = [
            [Cell(self.board[i][j], i, j, int(SQUARE_SIZE)) for j in range(height)]
    ...
    def remove_cells(self):  # Uses a counter to determine when the correct number of cells has been removed
        count = 0
        while count < self.difficulty:  # While the loop is True randomly generates positions within the board
